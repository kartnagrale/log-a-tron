...
<div class="shell" [class.sidebar-collapsed]="collapsed()">
 <header class="topbar">
  ...
  <div class="topbar-end"><span class="health-pill"><i></i>API connected</span><button class="icon-button theme-toggle" [attr.aria-label]="theme.isDark()?'Switch to light mode':'Switch to dark mode'" [attr.title]="theme.isDark()?'Light mode':'Dark mode'" (click)="theme.toggle()"><i [class]="theme.isDark()?'pi pi-sun':'pi pi-moon'"></i></button><div class="profile"><span><strong>{{auth.me()?.user?.displayName??'Loading identity'}}</strong><small>{{roleName()}}</small></span><span class="role-badge">{{primaryRole()}}</span><span class="avatar">{{initials()}}</span></div><p-button icon="pi pi-sign-out" severity="secondary" [text]="true" ariaLabel="Sign out" (onClick)="auth.logout()" /></div>
 </header>
 <aside class="sidebar"><nav aria-label="Primary navigation">@for(group of navGroups();track group.label){<small class="nav-section">{{group.label}}</small>@for(item of group.items;track item.route){<a [routerLink]="item.route" routerLinkActive="active"><i class="pi {{item.icon}}"></i><span>{{item.label}}</span></a>}}</nav><div class="sidebar-footer"><span class="status-dot"></span><div><strong>Managed evaluation stack</strong><small>v0.8 preview</small></div></div></aside>
 <main class="content"><router-outlet /></main>
</div>`})
export class ShellComponent {
 readonly auth=inject(AuthService);readonly capabilities=inject(CapabilityService);readonly scope=inject(ScopeContextService);readonly theme=inject(ThemeService);readonly collapsed=signal(false);
