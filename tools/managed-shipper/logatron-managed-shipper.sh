...
heartbeat(){
  ...
  body=$(printf '{"collectorVersion":%s,"appliedConfigHash":%s,"status":"%s","lastError":%s}' \
    "$(printf '%s' "$version" | json_escape)" "$(printf '%s' "$applied" | json_escape)" "$status" "$(printf '%s' "$error" | json_escape)")
...
apply_config_if_changed(){
  ...
  [[ -f "$HASH_FILE" ]] && applied="$(tr -d '\r\n' < "$HASH_FILE")"
  if [[ "$desired" == "$applied" ]] && [[ -f "$ACTIVE_CONFIG" ]]; then return 0; fi
...
while true; do
  if desired=$(fetch_config 2>>"$SHIPPER_LOG"); then
    if [[ -n "$desired" ]]; then apply_config_if_changed "$desired" || true
    else log "config response did not include X-Logatron-Config-Hash"; heartbeat DEGRADED "configuration response missing desired hash"; fi
  else
